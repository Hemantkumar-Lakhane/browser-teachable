...
import { predictImage, performLivePredictionStep, runBatchTest, generateTestReport } from './ml/prediction.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function syncPredictUploadState() {
    ...
    const predictBatchSelectBtn = document.getElementById('predictBatchSelectBtn');
    if (predictBatchSelectBtn) {
      predictBatchSelectBtn.disabled = !store.modelTrained;
    }
  }
    ...
    reader.onload = e => {
      ...
      predictPreview.onload = () => {
        ...
        if (predictUploadStatus) {
          predictUploadStatus.textContent = store.modelTrained ? `${file.name} is ready for prediction.` : `${file.name} is ready. Train the model to predict it.`;
        }
        syncPredictUploadState();
      };
  ...
  // Batch Test (Held-Out Test Set) Listeners
  const batchTestFolderInput = document.getElementById('batchTestFolderInput');
  const predictBatchSelectBtn = document.getElementById('predictBatchSelectBtn');
  const batchTestProgress = document.getElementById('batchTestProgress');
  const batchTestProgressBar = document.getElementById('batchTestProgressBar');
  const batchTestStatus = document.getElementById('batchTestStatus');
  const batchTestResults = document.getElementById('batchTestResults');
  const downloadTestReportBtn = document.getElementById('downloadTestReportBtn');
  if (predictBatchSelectBtn && batchTestFolderInput) {
    predictBatchSelectBtn.addEventListener('click', () => {
      batchTestFolderInput.click();
    });
  }
  if (batchTestFolderInput) {
    batchTestFolderInput.addEventListener('change', async e => {
      const files = e.target.files;
      if (!files || files.length === 0) return;
      batchTestProgress.style.display = 'block';
      batchTestResults.style.display = 'none';
      batchTestProgressBar.style.width = '0%';
      batchTestStatus.textContent = `Processing 0 / ${files.length} images...`;
      predictBatchSelectBtn.disabled = true;
      await runBatchTest(files, (processed, total) => {
        batchTestStatus.textContent = `Processing ${processed} / ${total} valid images...`;
        batchTestProgressBar.style.width = `${processed / total * 100}%`;
      }, results => {
        batchTestResults.style.display = 'block';
        predictBatchSelectBtn.disabled = false;
        batchTestFolderInput.value = ''; // Reset
      });
    });
  }
  if (downloadTestReportBtn) {
    downloadTestReportBtn.addEventListener('click', () => {
      generateTestReport();
    });
  }
  ...
  if (kagglePathInput && kaggleSizeLabel) {
    kagglePathInput.addEventListener('input', () => {
      kaggleSizeLabel.textContent = '';
    });
  }
  ...
  function showTrainingPanel() {
    ...
    if (trainingRunLog) trainingRunLog.setHTMLUnsafe('');
    showTrainingCancel();
  ...
  function updateTrainingProgress({
    ...
    if (typeof percent === 'number' && trainingProgressBar) trainingProgressBar.style.width = `${percent}%`;else if (epoch && totalEpochs && trainingProgressBar) {
  ...
  window.trainingFinished = info => {
    updateTrainingProgress({
      message: info?.message || 'Training complete',
      percent: 100
    });
  ...
  async function fetchKaggleZip(ownerSlug, datasetSlug) {
    ...
    try {
      const blob = await streamedFetch(directUrl, {
        headers: {
  ...
  async function processDatasetZip(blob, sourceLabel) {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      const relativePath = parts.length > 1 ? `${parts.slice(0, -1).join('/')}/${fileName}` : `Imported/${fileName}`;
      try {
  ...
  document.getElementById('trainBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      updateTrainingProgress({
        message: `Training error: ${err.message || err}`
      });
      // allow user to close
      showTrainingClose();
    }
  ...
  document.getElementById('predictImgBtn').addEventListener('click', () => {
    refreshWorkflowStep('predict');
    predictImage(predictPreview, {
      overlayEl: predictPreviewOverlay
    });
  });
  ...
  function stopLive() {
    if (store.predInterval) {
      clearInterval(store.predInterval);
      store.predInterval = null;
    }
  ...
  document.getElementById('resetBtn').addEventListener('click', () => {
    ...
    try {
      ...
      if (replayBars) replayBars.setHTMLUnsafe('');
      ...
      if (qb) qb.setHTMLUnsafe('<div class="qd-empty">Collect samples to see dataset quality insights here.</div>');
      if (preview) {
      ...
      if (distPairs) distPairs.setHTMLUnsafe('<div style="font-size:0.82rem;color:#a0aec0;">Add samples to at least 2 classes to see how separable they are.</div>');
