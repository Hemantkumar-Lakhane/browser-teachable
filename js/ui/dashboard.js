...
export async function renderQualityDashboard() {
  ...
  if (!hasAnySamples) {
    body.innerHTML = '<div class="qd-empty">Collect samples to see dataset quality insights here.</div>';
    return;
  }
  ...
  for (const cls of store.classes) {
    ...
    classSummaries.push({ cls, n, diversityScore, adviceType, advice });
  }

  const avgDiversity = diversityScores.length
  ...
  [
    ['Classes', `${nonEmptyClasses}/${store.classes.length}`],
    ['Samples', String(totalSamples)],
    ['Balance', balanceGood ? 'Good' : 'Needs work'],
    ['Variety', diversityScores.length ? `${avgDiversity}%` : 'Pending']
  ].forEach(([label, value]) => {
    const metric = document.createElement('div');
    metric.className = 'qd-metric';
    metric.innerHTML = `<span>${label}</span><b>${value}</b>`;
  ...
  classSummaries.slice(0, 10).forEach(({ cls, n, diversityScore, adviceType, advice }) => {
    const item = document.createElement('div');
  ...
  previewClasses.forEach(({ cls, n, diversityScore, adviceType, advice }) => {
