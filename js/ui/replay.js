...
export async function setReplaySource(src, label) {
  ...
  if (replaySourceInfo) replaySourceInfo.setHTMLUnsafe(`Test image: <b>${label}</b>. Drag the slider to scrub through epochs.`);
...
export async function scrubToEpoch(epochNum) {
  ...
  if (isEarly) {
    ...
    insight = converged ? `✅ <b>Epoch ${snap.epoch} — Converged.</b> Loss ${lossFmt}, accuracy ${accPct}%. The model confidently predicts <b>${winner?.name}</b> at ${conf}%.` : `🔄 <b>Epoch ${snap.epoch} — Still learning.</b> Loss ${lossFmt}, accuracy ${accPct}%. Not fully converged yet — more samples may help.`;
  }
  if (replayInsight) replayInsight.setHTMLUnsafe(insight);
}

// ── Auto-Play Controls ───────────────────────────────────────────

export function stopReplayAuto() {
  if (store.replayInterval) {
    clearInterval(store.replayInterval);
    store.replayInterval = null;
  }
  const replayPlayBtn = document.getElementById('replayPlayBtn');
...
export function syncReplayButtons() {
  ...
  const replaySnap = document.getElementById('replaySnap');
  const preview = document.getElementById('predictPreview');
  if (replayUseUpload) replayUseUpload.disabled = !(store.epochSnapshots.length && preview && preview.src && preview.naturalWidth > 0);
  if (replaySnap) replaySnap.disabled = !store.webcamReady;
}