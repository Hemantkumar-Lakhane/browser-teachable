...
const CONFIG_FIELDS = {
  epochs: { id: 'epochsInput', type: 'int', min: 1, max: 200 },
  batchSize: { id: 'batchSizeInput', type: 'int', min: 1, max: 256 },
  learningRate: { id: 'learningRateInput', type: 'float', min: 0.000001, max: 1 },
  optimizer: { id: 'optimizerSelect', type: 'string' },
  dropoutRate: { id: 'dropoutInput', type: 'float', min: 0, max: 0.8 },
  hiddenUnits: { id: 'hiddenUnitsInput', type: 'int', min: 8, max: 1024 }
};
...
export function initTrainingConfigControls(onBackboneChange) {
  ...
  if (backboneSelect) {
    backboneSelect.innerHTML = BACKBONE_OPTIONS.map(option =>
